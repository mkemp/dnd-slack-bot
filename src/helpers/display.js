...
function formatRoll(character, evaluations) {
    ...
    return {
        ...
        attachments: [
            {
                fallback: `<@${character.id}> rolled ${evaluations.map(evaluation => evaluation.fallback).join('; ')}`,
                text: `<@${character.id}> rolled ${evaluations.map(evaluation => evaluation.text).join('; ')}`,
                fields:  fields,
                mrkdwn_in: 'text'
            }
        ]
