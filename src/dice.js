...
RollDecorator.initEnum({
    Advantage: {
        decorate(roll_string) {
            return roll_string.replace('1d20', '2d20K1');
        }
    },
    None: {
        decorate(roll_string) {
    ...
    applyTo(result) {
        if (result.rolls.some(this.matchesCondition)) {
            result.rolls.forEach((value, index) => {
                let newValue = value;
                for (let i = 0; i < this.times && this.matchesCondition(newValue); i++) {
...
class Keep {
    ...
    applyTo(result) {
        ...
        result.rolls.splice(this.limit).forEach(value => result.removed.push(value));
        result.total = 0;
        result.rolls.forEach(value => result.total += value);
    }
}
...
class Term {
    ...
    toCanonicalForm() {
        return this.constant ? `${this.number}` :
            `${this.number}d${this.sides}${this.explode ? '!' : ''}${this.reroll || ''}${this.keep || ''}`;
    }
...
class Result {
    ...
    rollSingle() {
        return this.term.constant ? this.term.number : Math.floor(this.random() * this.term.sides) + 1;
    }
}
...
class Evaluation {
    constructor(expression, random=Math.random) {
        ...
        this.text = '';
        this.fallback = '';
        this.expression.terms.map(term => term.toResult(random)).forEach((result, index) => {
...
class Parser {
    ...
    static parse(text) {
        return text.split(splitExpressions).map((expression) => {
            ...
            const termsString = indexOfPurpose === -1 ? expression : expression.slice(0, indexOfPurpose).trim();
            const purpose = indexOfPurpose === -1 ? '' : expression.slice(indexOfPurpose + 5).trim();
            const terms = [];
            let match;
            while (match = splitTerms.exec(termsString)) {
                 ...
                 * 7: constant
                 */
                const [ _, operator, number, sides, explode, reroll, keep, constant ] = match;
                const term = new Term();
                ...
                if (Validation.isPresent(reroll)) {
                    ...
                    const times = Validation.isPresent(timesString) ? Math.max(1, parseInt(timesString)) : 1;
                    const value = Math.min(Math.max(1, parseInt(valueString)), term.sides);
                    term.reroll = (ltOrGt === '>') ? new ReRollHigherThan(times, value) : new ReRollLowerThan(times, value);
                }
                if (Validation.isPresent(keep)) {
                    const limit = Math.min(Math.max(1, parseInt(keep.slice(1), 10)), term.number);
                    term.keep = (keep.slice(0, 1) === 'K') ? new KeepHighest(limit) : new KeepLowest(limit);
                }
