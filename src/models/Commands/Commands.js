...
const { Skills } = require("../Character/Skill");
const { Stats } = require("../Character/Stat");
...
Commands.initEnum({
  Ability: {
    execute(character, text, format = formatRoll) {
      ...
      const [
        ...
        advantageName,
        purpose
      ] = this.buildParseRegex().exec(text);
      const evaluations = character.rollAbilityCheck(
        abilityName,
        purpose,
        RollDecorator.for(advantageName)
      );
    ...
    buildParseRegex() {
      return new RegExp(
        `^(?:(${allAbilities})(?:\\s+with ((?:dis)?adv(?:antage)?))?(?:\\s+(?:for\\s+)?)?([\\w ]*))$`,
        "gi"
      );
    },
    ...
    help() {
      return {
        ...
        attachments: [
          {
            text:
              "/ability {ability-name} [with [dis]adv[antage]] [[for] purpose]"
          }
        ]
      };
    }
  ...
  Attack: {
    execute(character, text, format = formatRoll) {
       ...
       * 3: purpose
       */
      const [_, attackName, advantageName, purpose] = this.buildParseRegex(
        character
      ).exec(text);
      const evaluations = character.rollAttackToHit(
        attackName,
        purpose,
        RollDecorator.for(advantageName)
      );
    ...
    buildParseRegex(character) {
      return new RegExp(
        `^(?:(${
          character.attacks.map(attack => attack.regex.source).join('|')
        })(?:\\s+with ((?:dis)?adv(?:antage)?))?(?:\\s+(?:for\\s+)?)?([\\w ]*))$`,
        "gi"
      );
    },
    ...
    help() {
      return {
        ...
        attachments: [
          {
            text:
              "/attack {attack-name} [with [dis]adv[antage]] [[for] purpose]"
          }
        ]
      };
    }
  ...
  Damage: {
    execute(character, text, format = formatRoll) {
       ...
       * 2: purpose
       */
      const [_, attackName, purpose] = this.buildParseRegex(character).exec(
        text
      );
  ...
  Initiative: {
    execute(character, text, format = formatRoll) {
      ...
      const evaluations = character.rollInitiative(
        purpose,
        RollDecorator.for(advantageName)
      );
      return format(character, evaluations);
    ...
    help() {
      return {
        ...
        attachments: [
          {
            text:
              "/roll {dice-expression} [for {purpose}][, {dice-expression} [for {purpose}]]"
          }
        ]
      };
    }
  ...
  Save: {
    execute(character, text, format = formatRoll) {
       ...
       * 3: purpose
       */
      const [_, statName, advantageName, purpose] = this.buildParseRegex().exec(
        text
      );
      const evaluations = character.rollSavingThrow(
        statName,
        purpose,
        RollDecorator.for(advantageName)
      );
    ...
    buildParseRegex() {
      return new RegExp(
        `^(?:(${allStats})(?:\\s+with ((?:dis)?adv(?:antage)?))?(?:\\s+(?:for\\s+)?)?([\\w ]*))$`,
        "gi"
      );
    },
