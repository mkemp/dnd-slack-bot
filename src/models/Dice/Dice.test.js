...
describe("Parser.parse()", function() {
  describe("handles single expression", function() {
    it("with purpose", function() {
      assert.equal(
        Parser.parse("2d6r<3 + 5 for damage").toString(),
        "2d6r1<3 + 5 for damage"
      );
    });
  ...
  describe("handles multiple expressions", function() {
    it("with purposes", function() {
      assert.equal(
        Parser.parse(
          "2d20K1 + 5 for hit; 2d6r<3 + 5 for damage; 1d4! for explosion"
        ).toString(),
    ...
    it("without purposes", function() {
      assert.equal(
        Parser.parse("2d20K1 + 5; 2d6r<3 + 5; 1d4!").toString(),
        "2d20K1 + 5,2d6r1<3 + 5,1d4!"
      );
    });
  });
});
...
describe("RollDecorator", function() {
  ...
  describe("Disadvantage.decorate()", function() {
    it("correctly replaces 1d20", function() {
      assert.equal(
        RollDecorator.Disadvantage.decorate("1d20 + 3"),
        "2d20k1 + 3"
      );
    });
...
describe("Roller.roll()", function() {
  it("evaluates each expression", function() {
    ...
    assert.deepEqual(Roller.roll(expressions), [
      "evaluation-1",
      "evaluation-2",
      "evaluation-3"
    ]);
  ...
  describe("with real values", function() {
    it("handles explode", function() {
      ...
      const evaluations = Roller.roll(Parser.parse("1d4! - 1"), () =>
        random.float()
      );
    ...
    it("handles re-rolls", function() {
      ...
      const evaluations = Roller.roll(Parser.parse("2d6r<3 + 6"), () =>
        random.float()
      );
    ...
    describe("keep high", function() {
      it("keeps the highest when the highest value is first", function() {
        const rolls = [18, 8];
        const evaluations = Roller.roll(Parser.parse("2d20K1 + 3"), () =>
          convertRollToFloat(rolls.shift(), 20)
        );
